...
- 🔐 **REALITY gateway (real Xray-core)** — per-subscription `VLESS + XHTTP + REALITY`
  inbound: pick your own **SNI/host**, listen port & dest port; keys are generated by
  Xray itself (`xray x25519`), rotated from the panel, and listeners start/reload automatically
...
- 🔐 گیتوی **REALITY واقعی روی هستهٔ Xray**: برای هر اشتراک اینباند `VLESS + XHTTP + REALITY` با انتخاب خودتِ SNI/هاست و پورت ورودی/خروجی؛ کلیدها توسط خود Xray تولید و لیسنرها خودکار اجرا/ریلود می‌شوند
